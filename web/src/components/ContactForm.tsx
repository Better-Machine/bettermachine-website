...
export function ContactForm({ subject, onClose, source }: ContactFormProps) {
  const [formData, setFormData] = useState({
    ...
    email: "",
    message: "",
    _subject: subject || "Contact from bettermachine.ai",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://formspree.io/f/xqeroapl", {
        method: "POST",
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
      ...
      <button
        ...
        className="w-full px-6 py-3 bg-[#B87333] text-white font-semibold rounded-lg
